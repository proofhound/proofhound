...
//
// Lets a deployment fold deployment-level runtime caps into a job's per-call RPM/TPM/concurrency
// limits just before enqueue — e.g. a SaaS org plan's concurrency ceiling. It carries NO billing
...
// caller's limits unchanged) and is invoked on every LLM enqueue, so OSS behavior is identical.
...
export interface RuntimeLimitsInput {
  ...
  modelId: string;
  /** The LLM job source (`experiment` / `optimization_*` / `release`) or a synchronous caller tag. */
  source: string;
