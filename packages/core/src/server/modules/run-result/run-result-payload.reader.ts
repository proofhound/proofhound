...
// it returns the inline value directly and never touches object storage. A replacement implementation binds its
// own implementation (resolving an offloaded `payload_ref` from its object store) in its `contracts`
// module; that implementation lives outside the OSS trunk.
