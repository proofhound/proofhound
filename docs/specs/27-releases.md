...
- `release_line_events.retention_days` controls release run-result rotation for that executable event. `NULL` retains its run results permanently; positive values such as 7 or 30 let the server-owned run-result retention sweeper delete that event's old rows after the configured age. The sweep path is advisory-lock guarded for multi-replica deployments and can be externally scheduled by setting `RUN_RESULT_RETENTION_SWEEP_MODE=external`. Manual run-result cleanup remains available from the release settings tab and through the run-result API/MCP tools.
...
- **Run results**: production and canary candidates uniformly write `source='release'`, `source_id=release_line_events.id`, and write `release_version_id`; dual-run is compared via `external_id`, `lane_type`, the release version, and the release event snapshot. Live release traffic only runs automatic judgment when the expected value can be retrieved from the input payload per `judgment_rules.rules[].expectedField` (default `expected_output`; historical `expected_field` remains readable); when there is no expected value, `judgment_status` is empty and the quality judgment is left to the annotation task, and a missing ground truth must not be recorded as `judge_error`.
