...
- `ph_runs.run_results` is a history fact table and is range-partitioned by `created_at` with monthly partitions. This keeps retention, archival, and time-bounded history queries manageable. It is not a quota mechanism: write paths must not synchronously scan `run_results`, dataset samples, or other detail tables to compute storage usage. Deployments that need quotas should maintain incremental counters / ledger entries and reconcile them asynchronously.
